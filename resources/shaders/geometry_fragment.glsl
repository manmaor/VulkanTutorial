...
vec3 calcNormal(float hasNormalMap, vec3 normal, vec2 textCoords, mat3 TBN)
{
    ...
    if (hasNormalMap > 0) {
        ...
        newNormal = normalize(TBN * newNormal); // Transforms to world coords
    }
    return newNormal;
}
...
//    outAlbedo = material.diffuseColor + texture(textSampler, inTextCoords);
    ...
    // Hack to about transparent PBR artifacts
    if (outAlbedo.a < 0.5) {
        discard;
    }

    // TBN = [T]angents, [B]itangents, [N]ormals
    mat3 TBN = mat3(inTangent, inBitangent, inNormal);
    vec3 newNormal = calcNormal(material.hasNormalMap, inNormal, inTextCoords, TBN);
    // Transform normals from [-1, 1] to [1, 0]
